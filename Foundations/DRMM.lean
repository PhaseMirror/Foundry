...
def weighted_sum (K : Nat) (energies : Fin K → Nat) (alpha : Nat) (primes : Fin K → Nat) : Nat :=
  sum_fin (fun i => (energies i * primes i * alpha) / 1000)
...
def step (c : Config) (d : Nat) (p_t : Fin d → MOC.MultiplicitySpace) (g_t : Fin d → MOC.MultiplicitySpace) (s_t : State d) 
         (primes : Fin (c.num_bins) → Nat) (U : Transform d) : (Fin d → MOC.MultiplicitySpace) × State d :=
  let spectrum := U g_t
...
theorem stability_of_recursion (c : Config) (d : Nat) (s : State d) 
  (p_t : Fin d → MOC.MultiplicitySpace) (primes : Fin c.num_bins → Nat) (U : Transform d) :
