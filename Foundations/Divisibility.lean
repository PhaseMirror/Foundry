/-!
# Multiplicity Kernel — Divisibility (ADR-0001 Phase 1 scope)

Elementary divisibility laws on `Nat`, used by primes, gcd/lcm and
factorization.  All facts are re-derived from `Nat`'s core arithmetic; no
algebra is imported.
-/
