/-!
# Foundations.Expr.Core — AST Expression Grammar & L0 Invariant Gates

Formalizes the syntax tree, L0 predicate validation framework, and bounds verification.
-/

namespace Foundations.Expr
...
/-- Theorem: Successor constructor rejects literal values > 1000000. -/
theorem trySuccessor_rejects_bounds (v : Nat) (h : v > 1000000) :
  ...
  have h_gt : (if v > 1000000 then Except.error "Bounds check violation in successor" else Except.ok ()) =
      Except.error "Bounds check violation in successor" := by simp [h]
  rw [h_gt]
  rfl

/-- Theorem: Stratum boundary constructor rejects zero boundary. -/
...
/-- Theorem: Prime shift constructor rejects base ≤ 1. -/
