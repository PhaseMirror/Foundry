/-!
# Foundations.Kernel.Factorization — Prime Valuations & Multiplicity Decompositions

Formalizes the $p$-adic valuation $\nu_p(n)$ (exponent of prime $p$ in $n$),
prime power divisibility bounds, valuation preservation laws, and factor list reconstructions.
-/

namespace Foundations.Kernel

/-- Structural recursive countdown search for largest power $p^k \mid n$. -/
def valuationAux (p : Nat) (n : Nat) : Nat → Nat
  | 0 => 0
  | k + 1 =>
      if p ^ (k + 1) ∣ n then k + 1 else valuationAux p n k
...
/-- Valuation of zero is zero. -/
theorem valuation_zero (p : Nat) : valuation p 0 = 0 := by
  dsimp [valuation]

/-- A power $p^a$ divides $p^b$ only when $a \le b$ (for $p \ge 2$). -/
...
/-- For $p \ge 2$, powers $p^k$ strictly exceed $k$. -/
...
/-- When $m \le k$, countdown search stops at $m$ for $n = p^k$. -/
...
/-- When $k < m$, countdown search stops at $k$ for $n = p^k$. -/
...
/-- Valuation of product of powers of same prime: $\nu_p(p^a \cdot p^b) = a + b$. -/
...
def factorProduct (fs : List Nat) : Nat := fs.foldl (· * ·) 1

/-- Product of empty list is 1. -/
theorem factorProduct_nil : factorProduct [] = 1 := rfl

/-- Product of singleton list $[x]$ is $x$. -/
theorem factorProduct_singleton (x : Nat) : factorProduct [x] = x := by
  dsimp [factorProduct]
  omega
