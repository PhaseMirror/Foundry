...
-- ===========================================================================
-- Accelerated sequence and trapezoidal correction for γ₁
-- ===========================================================================

-- ½ (ln k)^2
def halfLogSq (k : Nat) (hk : 1 ≤ k) : Real :=
  Rmul (ofQ (⟨1, 2⟩ : Q) (by decide)) (Rmul (logN k hk) (logN k hk))

/-- Trapezoidal-corrected sequence for γ₁.
Same limit `γ₁`, but error decays as `O(1/N^2)`. -/
def hSeq (j : Nat) : Real :=
  Rsub (gSeq j)
    (Rmul (ofQ (⟨1, 2⟩ : Q) (by decide)) (lnOver (j + 1) (Nat.succ_pos j)))

/-- Per-step trapezoidal residual for γ₁:
`s(p) = ½( f(p+1) + f(p) ) - ∫_p^{p+1} f(x) dx`
where `f(x) = (ln x)/x`, and `∫ f = ½(ln x)^2`.  Same pattern as in GammaTwoBracket. -/
def sStep (p : Nat) (hp : 1 ≤ p) : Real :=
  Rsub
    (Radd (Rmul (ofQ (⟨1, 2⟩ : Q) (by decide)) (lnOver (p + 1) (Nat.succ_pos p)))
