...
def field_degree (K : NumberField) : Nat := -- TODO: replace sorry

/-- A fractional ideal of O_K. -/
def FractionalIdeal := OK → Prop

/-- The principal ideal generated by an element α ∈ O_K. -/
def principal_ideal (alpha : OK) : FractionalIdeal := -- TODO: replace sorry

/-- The ideal class [I] of a fractional ideal I. -/
def ideal_class (I : FractionalIdeal) : Type := -- TODO: replace sorry

/-- The class group Cl(K) = Pic(O_K). -/
def class_group (K : NumberField) : Type := -- TODO: replace sorry

/-- The class number h_K = |Cl(K)|. -/
def class_number (K : NumberField) : Nat := -- TODO: replace sorry
...
def rational_prime_factorization (p : Nat) : List (PrimeIdeal × Nat) := -- TODO: replace sorry

/-- The splitting type of a prime p in O_K. -/
def splitting_type (p : Nat) : List Nat := -- TODO: replace sorry
...
def regulator (K : NumberField) : Float := -- TODO: replace sorry

/-- The number of roots of unity w_K in K. -/
def roots_of_unity (K : NumberField) : Nat := -- TODO: replace sorry

/-- The discriminant d_K of the number field. -/
def discriminant (K : NumberField) : Int := -- TODO: replace sorry
...
def class_number_formula_rhs (K : NumberField) : Float := -- TODO: replace sorry
...
def quadratic_field (d : Int) : NumberField := -- TODO: replace sorry

/-- The class number of Q(√d). -/
def quadratic_class_number (d : Int) : Nat := -- TODO: replace sorry

/-- The class number of Q(√-1) is 1 (Gaussian integers). -/
theorem gaussian_class_number_one : quadratic_class_number (-1) = 1 := by -- TODO: replace sorry

/-- The class number of Q(√-5) is 2. -/
theorem minus_five_class_number_two : quadratic_class_number (-5) = 2 := by -- TODO: replace sorry

/-- The class number of Q(√-163) is 1 (Heegner number). -/
theorem heegner_class_number_one : quadratic_class_number (-163) = 1 := by -- TODO: replace sorry

/-- The Kronecker symbol (d/n) generalizes the Legendre symbol. -/
def kronecker_symbol (d n : Int) : Int := -- TODO: replace sorry

/-- The Hilbert symbol (a,b)_p. -/
def hilbert_symbol (a b p : Nat) : Int := -- TODO: replace sorry
...
def ideal_density (K : NumberField) : Float := -- TODO: replace sorry

/-- The Minkowski bound: every ideal class contains an ideal of norm ≤ M_K. -/
def minkowski_bound (K : NumberField) : Nat := -- TODO: replace sorry
...
def kummer_ideal_as_dedekind (p : Nat) : FractionalIdeal := -- TODO: replace sorry
...
def order_of_vanishing_dedekind (n : Nat) (p : Nat) : Nat := -- TODO: replace sorry

/-- The motivic interpretation: prime ideals are points on Spec(O_K). -/
def spec_integers_points (K : NumberField) : List PrimeIdeal := -- TODO: replace sorry
