...
def term_symbol_string (term : TermSymbol) : String := -- TODO: replace sorry

/-- The number of microstates for a given electron configuration. -/
def electron_configuration_multiplicity (n_electrons : Nat) (n_orbitals : Nat) : Nat := -- TODO: replace sorry

/-! ### LS and jj Coupling -/

/-- LS coupling (Russell-Saunders): L and S are good quantum numbers. -/
def ls_coupling (L : OrbitalQuantumNumber) (S : SpinQuantumNumber) : TermSymbol := -- TODO: replace sorry

/-- jj coupling: individual electron angular momenta couple first. -/
def jj_coupling (electrons : List SpinOrbital) : List TermSymbol := -- TODO: replace sorry
