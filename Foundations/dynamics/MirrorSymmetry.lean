...
def gromov_witten_invariant (degree : Nat) (g : Nat) : Nat := -- TODO: replace sorry

/-- A period integral on the mirror Calabi-Yau manifold (analytic multiplicity). -/
def mirror_period (parameter : Nat) : Float := -- TODO: replace sorry
...
def yau_zaslow_formula (g : Nat) : Nat := -- TODO: replace sorry
...
def stable_map_moduli (degree : Nat) (g : Nat) : DerivedModuliStack := -- TODO: replace sorry
...
def bps_generating_function (invariants : List BPSInvariant) : Float := -- TODO: replace sorry
...
def donaldson_thomas_invariant (X : Scheme) : Nat := -- TODO: replace sorry
