...
def DRIFT_THRESHOLD_MAX : Q := { num := 17, den := 100 }
...
/-- Multiplier for each authority -/
def get_multiplier : Authority → Q
  | Authority.CA_FED => { num := 10, den := 10 }
  | Authority.CA_DEFENCE => { num := 12, den := 10 }
  | Authority.ITAR => { num := 15, den := 10 }
  | Authority.POLICY_QC => { num := 11, den := 10 }
  | Authority.OTHER => { num := 10, den := 10 }

/-- Effective threshold for an authority -/
