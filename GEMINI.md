...
- **Axiom-Clean Core:** All recursive stability proofs must be anchored to the canonical Lean 4 `MOC/Core.lean` (found in `lean/`) and satisfy the axiom-clean mandate: a self-contained core whose residual proof debt is explicitly manifested in `alp_sorry_manifest.json` (deadline + governor + paired witness per entry).
