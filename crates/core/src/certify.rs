...
pub fn certify_l0(records: &[StepInfo], tau: f64, tail_norm: f64, delta: f64) -> AceCertificate {
    if records.is_empty() {
        panic!("L0: no telemetry provided");
    }

    let min_epsilon = records
        .iter()
        .map(|r| r.epsilon)
...
pub fn ace_certificate(records: &[StepInfo], tail_norm: f64) -> AceCertificate {
