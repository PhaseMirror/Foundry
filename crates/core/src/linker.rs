    ...
    pub fn pass1_name_resolution(&mut self) -> Result<(), String> {
        for session in &self.config.sessions {
            for module_input in &session.modules {
                ...
                if !full_path.exists() {
                    return Err(format!(
                        "Pass 1: Bytecode not found for alias {}: {:?}",
                        module_input.alias, full_path
                    ));
                }
                ...
                let bytecode: PIRTMBytecode = serde_json::from_str(&content).map_err(|e| {
                    format!(
                        "Pass 1: Invalid bytecode format for {}: {}",
                        module_input.alias, e
                    )
                })?;

                self.resolved_modules
                    .insert(module_input.alias.clone(), bytecode);
            }
        }
        Ok(())
    }
    ...
    pub fn pass3_spectral_gate(&self) -> Result<GovernanceSeal, String> {
        ...
        for session in &self.config.sessions {
            ...
            if session.coupling_matrix.len() != dim {
                return Err(format!(
                    "Pass 3: Session {}: coupling matrix size mismatch",
                    session.name
                ));
            }
            for (i, row) in session.coupling_matrix.iter().enumerate() {
                if row.len() != dim {
                    return Err(format!(
                        "Pass 3: Session {}: coupling row {} size mismatch",
                        session.name, i
                    ));
                }
