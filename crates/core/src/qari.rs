...
use crate::gate::{CustomPredicate, EmissionGate, EmissionPolicy, GatedOutput};
...
impl QARISession {
    pub fn new(
        ...
        let p_op = projector.unwrap_or(|x| x.clone());
    ...
    pub fn step(
        ...
        if self.step_count >= self.config.max_steps {
            return Err(format!(
                "QARISession exceeded max_steps ({})",
