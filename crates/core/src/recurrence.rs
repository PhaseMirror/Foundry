use crate::projection::project_parameters_soft;
use crate::types::{PrimeMask, ResonanceWord};
use crate::types::{Status, StepInfo};
...
pub fn step(
    ...
    let resonance_word = if t < 64 {
        Some(ResonanceWord::pack(
            (t % 96) as u8,
            (residual.to_bits() >> 7) & ((1 << 57) - 1),
        ))
    } else {
        None
    };
...
pub fn fixed_point_estimate(history: &[DVector<f64>], window: usize) -> (DVector<f64>, f64) {
    if history.is_empty() {
