...
## Next Steps
1. Expand `lean/SNAPKITTY/SnapKitty/Core.lean` to formally define `ThermalWindow`, `call49`, and `VonNeumannEntropy`.
2. Extract these bounds via `build.rs` to inform the Rust UAC controller.
3. Update the `QCFI` Rust implementation to respect the compiled `ThermalWindow` constraints.
