...
theorem qRoundUp_ge (q : Q) (hqd : 0 < q.den) (D : Nat) : Qle q (qRoundUp q D) := by
  ...
  have hmod := Int.emod_def (q.num * (D : Int)) (q.den : Int)
...
theorem gSeq_block_ge (a : Nat) :
    Rle (Rneg (ofQ (⟨(a + 2 : Int), 2 ^ a⟩ : Q) (Nat.pow_pos (by decide : 0 < 2))))
  ...
  exact Rle_trans
    ...
    (Rle_ofQ_ofQ hmid (Nat.pow_pos (by decide : 0 < 2))
...
theorem Wsum_den_pos (A : Nat) : ∀ e, 0 < (Wsum A e).den
  | 0 => Nat.one_pos
  | (e + 1) => add_den_pos (Wsum_den_pos A e) (Nat.pow_pos (by decide : 0 < 2))
...
theorem Wsum_tail_le (A : Nat) : ∀ e,
  ...
  | (e + 1) => by
      have hT : 0 < (Qsub (⟨(2 * A + 6 : Int), 2 ^ A⟩ : Q) ⟨(2 * (A + e) + 6 : Int), 2 ^ (A + e)⟩).den :=
        Qsub_den_pos (Nat.pow_pos (by decide : 0 < 2)) (Nat.pow_pos (by decide : 0 < 2))
...
theorem gamma_pair_le {j k : Nat} (hjk : j ≤ k) :
  ...
  exact Qle_trans (Nat.mul_pos (by decide) (Nat.pow_pos (by decide : 0 < 2)))
