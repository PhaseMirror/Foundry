...
theorem g3_TU_le (j : Nat) :
    Qle (⟨(6 * gamma3Midx j * gamma3Midx j + 36 * gamma3Midx j + 66 : Int), 2 ^ gamma3Midx j⟩ : Q)
        ⟨1, j + 1⟩ := by
  have h := g3_domination_U j
  simp only [Qle, gamma3Midx]
  have nat_le : (6 * (2 * j + 14) * (2 * j + 14) + 36 * (2 * j + 14) + 66) * (j + 1)
      ≤ 1 * 2 ^ (2 * j + 14) := by omega
  exact_mod_cast nat_le
  ...
  | succ m ih =>
      ...
      have hexp : 48 * (m + 1) * (m + 1) + 864 * (m + 1) + 3916
        ...
        have hi : ((48 * (m + 1) * (m + 1) + 864 * (m + 1) + 3916 : Nat) : Int)
            = (((48 * m * m + 864 * m + 3916) + (96 * m + 912) : Nat) : Int) := by
          set_option maxRecDepth 10000 in push_cast; ring_uor
        exact_mod_cast hi
      have hlin := g3_linL m
      rw [hp, hexp]; omega
...
theorem g3_TL_le (j : Nat) :
  ...
  have h := g3_domination_L j
  simp only [Qle, gamma3Midx]
  have nat_le : (2 * (2 * j + 14) * (2 * j + 14) * (2 * j + 14) + 18 * (2 * j + 14) * (2 * j + 14)
        + 66 * (2 * j + 14) + 102) * (j + 1) ≤ 1 * 2 ^ (2 * j + 14) := by omega
  exact_mod_cast nat_le
