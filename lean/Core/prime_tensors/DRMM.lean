...
def weighted_sum (K : Nat) (energies : Fin K → Nat) (alpha : Nat) (primes : Fin K → Prime) : Nat :=
...
def step (c : Config) (d : Nat) (p_t : Fin d → MOC.MultiplicitySpace) (g_t : Fin d → MOC.MultiplicitySpace) (s_t : State d) 
         (primes : Fin (c.num_bins) → Prime) (U : Transform d) : (Fin d → MOC.MultiplicitySpace) × State d :=
  let spectrum := U g_t
...
theorem stability_of_recursion (c : Config) (d : Nat) (s : State d) 
  (p_t : Fin d → MOC.MultiplicitySpace) (primes : Fin c.num_bins → Prime) (U : Transform d) :
