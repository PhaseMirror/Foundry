...
noncomputable def braid (w : OperatorWord) : OperatorWord :=
  ...
  { ops := reducedOps,
    ...
    grade_ok := by
      -- Preservation of grading follows from construction; we sketch proof.
      have h : reducedOps.all (fun op => prime_grading op = some w.prime_grade) :=
        by
          intro op hmem
          have : op ∈ w.ops ∨ op = MocOperator.S_p ⟨w.prime_grade, ?_⟩ := sorry
          sorry
      exact h }
...
theorem resonance_functional_bounded (H : Hamiltonian) (ops : List MocOperator) (D : Domain) :
  ...
  have h0 : 0 ≤ (ops.map operator_weight).foldl (· + ·) 0.0 := by
    apply List.foldl_nonneg; intro x hx; simp [operator_weight]; positivity
  have hnorm : 0 ≤ (ops.map operator_weight).foldl (· + ·) 0.0 / (D.dimension.toReal + 1.0) :=
    div_nonneg h0 (by positivity)
  constructor
  · exact le_of_lt (lt_of_le_of_lt hnorm (by linarith))
  · apply le_of_lt
    have : (ops.map operator_weight).foldl (· + ·) 0.0 / (D.dimension.toReal + 1.0) ≤ 1.0 :=
      by
        have : (ops.map operator_weight).foldl (· + ·) 0.0 ≤ D.dimension.toReal + 1.0 := by sorry
        have : (ops.map operator_weight).foldl (· + ·) 0.0 / (D.dimension.toReal + 1.0) ≤ 1.0 :=
          div_le_one_of_le (by positivity) this
        exact this
    exact le_trans (by apply min_le_left) this
