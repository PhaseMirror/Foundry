namespace Multiplicity.Core.F1.Square.ADR099
...
def Submodule (_α : Type) (β : Type) := β → Prop

def HeckeSpan (_r : Nat) (_A : Nat → Matrix (Fin _r) (Fin _r) Float) : Submodule Float (Matrix (Fin _r) (Fin _r) Float) :=
  fun _ => True

def DiagonalComplement (_d : Nat) : Submodule Float (Matrix (Fin _d) (Fin _d) Float) :=
  fun M => ∀ i, M i i = 0.0
...
def norm_F (_X : Matrix (Fin d) (Fin d) Float) : Float := 0.0
...
def spectral_margin (_H : Matrix (Fin d) (Fin d) Float) : Float := 0.0

def dot_matrix (_v : Fin d → Float) (_w : Fin d → Float) : Float := 0.0
def mul_vec (_M : Matrix (Fin d) (Fin d) Float) (_v : Fin d → Float) : Fin d → Float := fun _ => 0.0
def conjTranspose (v : Fin d → Float) : Fin d → Float := v
def vec (_M : Matrix (Fin d) (Fin d) Float) : Fin d → Float := fun _ => 0.0

theorem mode3_wrapper_correct
  (d : Nat) (atlasM : Matrix (Fin d) (Fin d) Float)
  (H : Matrix (Fin d) (Fin d) Float)
  (HeckeOperator : Nat → Matrix (Fin d) (Fin d) Float)
  (_hH_span : HeckeSpan d HeckeOperator H)
  (_hH_neg : ∀ v : Fin d → Float, v ≠ (fun _ => 0.0) → (∀ i, v i = 0.0) →
      (dot_matrix (conjTranspose v) (mul_vec H v) < 0.0))
  (η : Float) (_h_eta : η < spectral_margin H)
  (Δ : Matrix (Fin d) (Fin d) Float)
  (_h_dist : norm_F (sub_matrix Δ H) ≤ η)
  (h_res : ∃ (subspace : Submodule Float (Matrix (Fin d) (Fin d) Float)),
  ...
  ∃ (subspace : Submodule Float (Matrix (Fin d) (Fin d) Float)),
    ∀ M : Matrix (Fin d) (Fin d) Float, subspace M → M ≠ (fun _ _ => 0.0) → (∀ i, M i i = 0.0) → dot_matrix (conjTranspose (vec M)) (mul_vec Δ (vec M)) < 0.0 := h_res
...
theorem atlasM_Mode3_wrapper
  ...
  exact mode3_wrapper_correct d atlasM H HeckeOperator hH_span hH_neg η h_eta Δ h_dist h_res

end Multiplicity.Core.F1.Square.ADR099
