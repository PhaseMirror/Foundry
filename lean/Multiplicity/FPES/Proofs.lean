...
the FPES core.  **No `sorry`, no `admit`, no axioms**: the compiler rejects
...
accepts the proof term; there is no `sorry`. -/
@[fpes_proof]
...
image `p' ∈ op(H).paths` with `equivalent_mechanism p p'` — no `sorry`.
