...
/-!
  ...
  If the Hodge-type intersection pairing on the cohomology of the arithmetic 
  surface is negative definite on the subspace spanned by the prime-indexed operators, 
  then the associated CPTP channel Φ is strictly contractive (ρ_ess(Φ) < 1).
