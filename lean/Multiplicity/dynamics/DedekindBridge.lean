...
def field_degree (K : NumberField) : Nat := sorry

/-- A fractional ideal of O_K. -/
def FractionalIdeal := OK → Prop

/-- The principal ideal generated by an element α ∈ O_K. -/
def principal_ideal (alpha : OK) : FractionalIdeal := sorry

/-- The ideal class [I] of a fractional ideal I. -/
def ideal_class (I : FractionalIdeal) : Type := sorry

/-- The class group Cl(K) = Pic(O_K). -/
def class_group (K : NumberField) : Type := sorry

/-- The class number h_K = |Cl(K)|. -/
def class_number (K : NumberField) : Nat := sorry
...
def rational_prime_factorization (p : Nat) : List (PrimeIdeal × Nat) := sorry

/-- The splitting type of a prime p in O_K. -/
def splitting_type (p : Nat) : List Nat := sorry
...
def regulator (K : NumberField) : Float := sorry

/-- The number of roots of unity w_K in K. -/
def roots_of_unity (K : NumberField) : Nat := sorry

/-- The discriminant d_K of the number field. -/
def discriminant (K : NumberField) : Int := sorry
