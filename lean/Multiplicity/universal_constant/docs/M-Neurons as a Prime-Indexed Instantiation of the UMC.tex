...
\title{M-Neurons as a Prime-Indexed Instantiation of the Universal Multiplicity Constant
\(\Lambda_m\): \\
...
It formalizes M-Neurons as a prime-indexed, multiplicity-theoretic neural architecture that realizes the Universal Multiplicity Constant \(\Lambda_m\) on a finite-dimensional Hilbert space.
Starting from a generic \(\Lambda_m^{\mathrm{op}}(t)\)–\(\Xi(t)\) evolution on a separable Hilbert space, we build a concrete M-Neuron model with prime-labeled channels, multiplicity norms, and a cubic nonlinearity that induces a dissipative quartic term in a Lyapunov functional.
We prove discrete and continuous-time stability theorems in the multiplicity norm, show how the M-Neuron satisfies the specification's boundedness and contraction conditions via a dissipative polynomial + small-coupling structure (rather than global Lipschitzness), and provide a runnable PyTorch implementation plus a prime-factor completion experiment.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{M-Neuron as \(\Lambda_m\) Interface.}
  We construct the M-Neuron as a finite-dimensional, prime-indexed realization of the abstract \(\Lambda_m^{\mathrm{op}}(t)\)–\(\Xi(t)\) evolution described in a separate working specification (DOCUMENT id="4xFSz"). The M-Neuron's global state lives in a prime-indexed Hilbert space \(H \simeq \mathbb{R}^{N K}\), with \(K\) prime channels per neuron.
  ...
  \begin{itemize}
    \item Discrete: existence of an explicit invariant ball and a uniform contraction factor \(k<1\) for the M-Neuron map \(\Phi_t(\Psi) = \Xi(t)\Psi + \lambda_m(t)T(\Psi)\).
  ...
  A PyTorch implementation of M-Neurons is provided, along with a prime-factor completion experiment (\(n_{t+1}=6n_t\)) comparing M-Neurons to a GRU baseline on prime-multiplicity encoded data.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item The M-Neuron architecture as a prime-indexed, multiplicity-normed neural system realizing the \(\Lambda_m\) specification.
  ...
  \item A concrete algorithmic realization in PyTorch, plus a structured prime-factor prediction task where M-Neurons confer a multiplicative inductive bias.
\end{itemize}
...
Let \(P = \{p_1,\dots,p_K\}\) be a finite set of primes. We consider a network of \(N\) M-Neurons.
...
In this M-Neuron instantiation, we take it to be scalar:
\begin{equation}
...
\subsection{Discrete and Continuous M-Neuron Dynamics}

\paragraph{Discrete time.}
The discrete-time M-Neuron evolution is
\begin{equation}
...
The continuous-time M-Neuron dynamics is given by the ODE
\begin{equation}
...
We now state the main stability theorems that certify the M-Neuron dynamics as a well-posed, exponentially contracting realization of the \(\Lambda_m\) specification, despite using a non-globally-Lipschitz cubic nonlinearity.

\subsection{Discrete-Time M-Neuron: Invariant Ball and Contraction}

\begin{theorem}[Discrete M-Neuron with Cubic Nonlinearity: Existence, Boundedness, and Exponential Contraction]
\label{thm:discrete}
...
\subsection{Continuous-Time M-Neuron: Global Existence and Dissipative Stability}

\begin{theorem}[Continuous M-Neuron ODE: Global Existence and Dissipative Stability]
...
\subsection{M-Neuron Layer in PyTorch}

We now give a concrete PyTorch implementation of the discrete M-Neuron dynamics, together with a simple readout layer for prediction tasks.

\begin{lstlisting}[language=Python,caption={M-Neuron implementation in PyTorch}]
import torch
...
class NPMultiplicityNeuron(nn.Module):
    """
    M-Neuron: Λ_m^{op}(t) Ξ(t) evolution with cubic T on a prime-indexed space.
    """
    ...
    def forward(self, v_in, steps=3):
        ...
        Unrolls 'steps' internal M-Neuron updates and returns a prediction
...
We train both M-Neuron and GRU on the same batch stream and compare mean squared error:

\begin{lstlisting}[language=Python,caption={Training M-Neuron and GRU on prime-factor completion}]
...
plt.plot(smooth(np_losses), label='M-Neuron')
plt.plot(smooth(gru_losses), label='GRU baseline')
plt.xlabel('Epoch')
plt.ylabel('Smoothed MSE loss')
plt.title('Prime-factor completion: M-Neuron vs GRU')
plt.legend()
plt.tight_layout()
plt.show()
\end{lstlisting}

Empirically, M-Neurons are expected to learn faster and reach lower error than GRUs on this structured multiplicative task, since the prime-indexed architecture aligns directly with the arithmetic structure of the data.
...
\begin{itemize}[leftmargin=*]
  ...
  \item M-Neuron-like architectures implemented in software (e.g.\ PyTorch, JAX) with:
  \begin{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  Port the M-Neuron to JAX with \(\#P\) up to 16 or 32 primes and compare scaling behaviour and expressivity on more complex multiplicative tasks.
...
Let \(H \simeq \mathbb{R}^{N K}\) denote the global state space of the M-Neuron network, where \(N\) is the number of neurons and \(K = |P|\) is the number of prime channels. We write
\[
...
\section{Discrete-Time M-Neuron: Detailed Proof of Theorem~\ref{thm:discrete}}

We restate the discrete M-Neuron map:
\[
\Phi_t(\Psi) = \Xi(t)\Psi + \lambda_m(t)T(\Psi),
\]
with assumptions:
...
\section{Continuous-Time M-Neuron: Detailed Proof of Theorem~\ref{thm:continuous}}
...
The M-Neuron construction is prime-channel block-diagonal in the sense that both \(\Xi(t)\) and the nonlinear operator \(T\) act in a way that respects the prime index structure.
