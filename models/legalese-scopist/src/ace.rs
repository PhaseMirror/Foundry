use crate::{EsiInputs, UnifiedWitness, evaluate_esi_risk};
...
pub fn evaluate_esi_risk_with_ace(
    ...
    // Modeling cost: WHT-Epistasis scalable in-place transform cost O(M log M)
    let zero_count = inputs.zeta_truncation.max(1); // avoid 0 division semantics if absent
    let modeling_cost = (384.0 * (zero_count as f64 / 20.0).ceil()) as u64;
    
    if inputs.histogram.len() == 64 {
        envelope.consume_ops(modeling_cost)?;
    }

    // Execute the computationally heavy engine logic
    let witness = crate::evaluate_esi_risk(inputs, p_factor, sigma, envelope.ops_consumed);
...
mod verification {
    ...
    fn verify_ace_envelope_budget_invariant() {
        ...
        if result.is_ok() {
            ...
            kani::assert(
                !envelope.is_exhausted,
                "Envelope must not be exhausted on success",
            );
            kani::assert(
                envelope.ops_consumed <= budget.max_ops,
                "Ops consumed MUST strictly respect max_ops boundary",
            );
        } else {
            // If failure, envelope exhaustion must trigger
            kani::assert(
                envelope.is_exhausted,
                "Envelope MUST be permanently exhausted on budget breach",
            );
        }
    }
}
