...
mod tests {
    ...
    fn test_executor_prime_17() {
        ...
        assert!(
            trace.rows.len() >= 10,
            "n=17 should have >= 10 rows, got {}",
            trace.rows.len()
        );

        // Check at least one row has Barrett columns populated
        let has_witness = trace.rows.iter().any(|r| r.mu_lo > 0 || r.mu_hi > 0);
        ...
        assert!(
            has_mul || has_square,
            "No multiply or square operations recorded"
        );
    }
    ...
    fn test_executor_composite_561() {
        ...
        let all_zeros = trace
            .rows
            .iter()
            .all(|r| r.mu_lo == 0 && r.mu_hi == 0 && r.t_lo == 0 && r.t_hi == 0);
...
fn test_is_end_on_early_exit() {
    ...
    assert_eq!(
        end_count, 2,
        "Should have exactly 2 is_end markers, got {}",
        end_count
    );
