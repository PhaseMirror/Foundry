...
fn air_json_column_order_frozen() {
    ...
    let contents = fs::read_to_string(air_path).expect("air.json must exist at repository root");

    let air: Value = serde_json::from_str(&contents).expect("air.json must be valid JSON");

    let columns = air["columns"]
        .as_array()
        .expect("air.json must have 'columns' array");

    assert_eq!(columns.len(), 20, "air.json must define exactly 20 columns");
...
fn air_json_commitment_scheme_keccak() {
    ...
    let commitment = air["commitment_scheme"]
        .as_str()
    ...
    let transcript = air["transcript_hash"]
        .as_str()
