...
impl PolicyEngine {
    ...
    pub fn validate_action(
        &self,
        action: &Action,
        trust: &materia_commons::types::TrustLevel,
        ...
        if let Some(ref binding) = action.server_binding {
            if binding.contains("external")
                && matches!(trust, materia_commons::types::TrustLevel::External)
            {
...
pub mod policy {
    pub use materia_commons::types::TrustLevel;
}
...
pub fn evaluate_preservation(json_input: &str) -> Result<String, anyhow::Error> {
    ...
    {
        materia_commons::types::TrustLevel::External
    } else {
        materia_commons::types::TrustLevel::Internal
    };
