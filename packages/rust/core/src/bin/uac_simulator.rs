...
fn main() {
    ...
    let initial_coverage = args
        .iter()
        .position(|a| a == "--initial-coverage")
    ...
    println!(
        "Langlands Loss (Arithmetic Deviation): {:.4}",
        initial_langlands
    );
    ...
    let (pre_defect_val, post_defect_val, pre_rta_val, post_rta_val) =
        if let Some(cell_path) = cell_path {
            ...
            let coherence: Vec<f64> = config["coherence"]
                .as_array()
                .unwrap()
                .iter()
                .map(|v| v.as_f64().unwrap())
                .collect();
            let defect_rows: Vec<Vec<f64>> = config["defect"]
                .as_array()
                .unwrap()
                .iter()
                .map(|r| {
            ...
            let cell = LinearMultiplicityCell::new(
                ...
                Array2::from_shape_vec(
                    (defect_rows.len(), defect_rows[0].len()),
                    defect_rows.concat(),
                )
                .unwrap(),
            );
    ...
    println!(
        "Langlands Loss (Arithmetic Deviation): {:.8}",
        final_langlands
    );
