...
pub struct SessionOrchestrator {
    sessions: HashMap<String, QARISession>,
...
impl SessionOrchestrator {
    ...
    pub fn get_mut(&mut self, session_id: &str) -> Option<&mut QARISession> {
        self.sessions.get_mut(session_id)
    }

    pub fn aggregate_certificates(
        &mut self,
        session_ids: Option<Vec<String>>,
        tail_norm: f64,
    ) -> Result<AggregatedCertificate, String> {
    ...
    pub fn global_summary(&self) -> serde_json::Value {
        let per_session: HashMap<String, serde_json::Value> = self
            .sessions
            .iter()
            .map(|(sid, s)| (sid.clone(), s.summary()))
            .collect();

        let total_steps: usize = per_session
            .values()
            .map(|v| v["steps"].as_u64().unwrap_or(0) as usize)
            .sum();

        let total_projections: usize = per_session
            .values()
