...
pub fn neutrality_check(
    ...
    if subjects.len() < 2 {
        return (true, serde_json::json!({"pairs_checked": 0, "max_deviation": 0.0, "violations": []}));
    }
...
pub fn beneficence_check(
    ...
    let growth = if norm_t > 0.0 { norm_next / norm_t } else { norm_next };
...
pub fn evaluate_csl(
    ...
    let (beneficence_ok, beneficence_detail) = beneficence_check(x_t, x_next, info, norm_growth_limit, residual_limit);
