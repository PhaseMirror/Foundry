...
fn resolve_alphas(primes: &[u64], profile: WeightProfile) -> Vec<f64> {
    ...
    match profile {
        ...
        WeightProfile::Harmonic => (1..=count).map(|i| (1.0 / i as f64).min(1.0).max(0.0)).collect(),
        WeightProfile::LogDecay => {
            let raw: Vec<f64> = primes.iter().map(|&p| 1.0 / (p as f64).max(2.0).ln().max(1e-12)).collect();
            let min = raw.iter().fold(f64::INFINITY, |a, &b| a.min(b));
            let max = raw.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
            if max - min < 1e-12 {
                vec![0.5; count]
            } else {
                raw.iter().map(|&r| ((r - min) / (max - min)).min(1.0).max(0.0)).collect()
            }
        }
        WeightProfile::Custom(f) => primes.iter().enumerate().map(|(i, &p)| f(i + 1, p).min(1.0).max(0.0)).collect(),
    }
}
