...
impl FermatCertifier {
    pub fn issue(op: &MocOperator) -> Result<ContractionCertificate, CertError> {
        if op.spectral_radius >= 1.0 {
            return Err(CertError::NonContractive(
                op.name.clone(),
                op.spectral_radius,
            ));
        }
        ...
        let proof_hash = blake3::hash(
            format!("{}_{}_{}", op.name, op.prime_gate, op.spectral_radius).as_bytes(),
        )
        .to_string();
    ...
    pub fn verify(cert: &ContractionCertificate) -> bool {
        ...
        let expected_hash = blake3::hash(
            format!(
                "{}_{}_{}",
                cert.operator_name, cert.prime_gate, cert.lambda_p
            )
            .as_bytes(),
        )
        .to_string();
...
mod verification {
    ...
    fn proof_certificate_soundness() {
        ...
        match FermatCertifier::issue(&op) {
            Ok(cert) => {
                kani::assert(
                    cert.lambda_p < 1.0,
                    "Issued certificate for non-contractive operator",
                );
                kani::assert(
                    FermatCertifier::verify(&cert),
                    "Issued certificate fails verification",
                );
            }
            Err(_) => {
    ...
    fn proof_resonance_preserves_contraction() {
        ...
        kani::assert(
            new_op.spectral_radius < 1.0,
            "Resonance broke contractivity bounds",
        );
    }
}
