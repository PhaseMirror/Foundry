...
use multiplicity_core::red_team::modification_safety_check;
...
impl SelfModificationDaemon {
    ...
    pub async fn execute(
        ...
        // Phase 3: Gates
        // Phase 1.5: Red‑Team safety check
if let Err(e) = modification_safety_check(proposal) {
    return ExecutionResult {
        status: "blocked".to_string(),
        proposal_id: proposal.proposal_id.clone(),
        pre_snapshot_id: None,
        post_snapshot_id: None,
        reason: format!("Red‑Team safety reject: {}", e),
        watchdog_checks: 0,
    };
}
