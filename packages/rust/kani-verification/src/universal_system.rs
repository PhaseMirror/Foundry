...
mod verification {
    ...
    fn verify_tnn_layer_stability() {
        ...
        assert!(
            regulated_proj <= linear_proj as u64,
            "TNN failed to compress the recursive layer space!"
        );
    ...
    fn verify_adaptive_learning_step() {
        ...
        if regulator_fraction == 0 {
            assert!(
                w_next == w_t,
                "Weights must precisely freeze if multiplicity regulators evaluate to 0"
            );
        } else {
            assert!(
                w_next >= w_t,
                "Multiplicity learning step must be monotonically additive under positive scaling"
            );
        }
    }
}
