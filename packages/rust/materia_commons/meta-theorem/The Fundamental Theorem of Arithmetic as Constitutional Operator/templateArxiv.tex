...
\geometry{margin=1in}
\pagestyle{fancy}
\lhead{FTA as Constitutional Operator}
\rhead{\thepage}
...
{\Huge\bfseries
$\Lambda_{\mathrm{m}}\Xi(t)$ \par}

\vspace*{1em}
  \Huge\textbf{Prime-Indexed Recursive Tensor Mathematics (PIRTM)}
   \vspace*{0.5em}
    
    {\Large\bfseries The Fundamental Theorem of Arithmetic as Constitutional Operator \par}
...
\thispagestyle{empty}
\begin{center}
    \Large\textbf{From Primes to Consciousness: One Operator Chain}
\end{center}
...
In the construction and stewardship of lawful systems—whether cognitive, technological, or organizational—we adopt the following foundational principles as the ethical substrate upon which all prime recursion shall unfold. These principles do not stand outside the system as optional commentary; they define the admissible conditions under which recursive processes may evolve, stabilize, and scale without collapse. Accordingly, every operator, feedback loop, and governance layer introduced in this work is to be understood as ethically bounded, structurally accountable, and subordinate to the preservation of lawful multiplicity.
...
\texttt{DOI: 10.13140/RG.2.2.25352.79364}} 
\date{May 4, 2026}
\maketitle
...
The FTA states that every integer \(n > 1\) factors uniquely as \(n = \prod_p p^{k_p}\) where \(p\) ranges over primes and exponents \(k_p \geq 0\) are finite-support \cite{HardyWright1979}. This uniqueness guarantee, trivial in elementary number theory, becomes \emph{the} structural invariant preventing collapse when extended to recursive systems.
...
\center
 \vspace{10cm}
     \LARGE{One tiny step for man, one quantum leap for mankind... \par}
    \vfill
