...
//! Library entry point for the Phase‑Mirror agent.
//! This file provides a reusable async `run_agent` function that can be
//! invoked from integration tests or other Rust crates. The binary `main`
//! function (in `src/main.rs`) simply calls this function.
...
async fn start_session(Extension(state): SharedState) -> impl IntoResponse {
    ...
    Json(json!({"session": lock.session_id}))
}
...
async fn did(Extension(state): SharedState) -> impl IntoResponse {
    ...
    if let Some(kp) = &lock.keypair {
        ...
        Json(json!({"did": pub_b64.0}))
    } else {
        Json(json!({"error": "keypair not initialized"}))
    }
}

#[axum::debug_handler]
async fn ws_handler(ws: WebSocketUpgrade, Extension(_state): SharedState) -> impl IntoResponse {
    ...
    ws.on_upgrade(move |socket| async move {
        ...
        while let Some(Ok(msg)) = receiver.next().await {
            if let Message::Text(txt) = msg {
                ...
                if let Some(tool) = cnl_parse(txt.as_str()) {
...
/// Runs the Phase‑Mirror agent server.
///
/// This function is used by the binary entry point (`src/main.rs`) and can also be
/// invoked directly from integration tests.
