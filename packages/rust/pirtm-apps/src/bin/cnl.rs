        ...
        match action {
            VerifiedAction::Deploy {
                ...
                self.active
                    .lock()
                    .await
                    .insert(action_id.clone(), container_ids);
                Ok(action_id)
            }
...
impl DialogueFrame {
        ...
        if mode == Mode::Assert {
            if let Err(e) = PhaseMirrorInvariants::enforce_all(&new_stratum) {
                ...
                if let Some(action) = &parsed_action {
                    ...
                    let allowed = alp_evaluate(
                        &action_id,
                        matches!(action, VerifiedAction::Deploy { .. }),
                        binding.as_deref(),
                    )
                    .await;
                    if !allowed {
                        println!(
                            "❌ ALP Blocked: action {} not admitted by policy engine",
                            action_id
                        );
                    } else {
            ...
            if let Some(action) = &parsed_action {
                ...
                if !allowed {
                    println!(
                        "❌ ALP Blocked: retraction {} not admitted by policy engine",
                        action_id
                    );
                } else {
                    match self.executor.execute(action).await {
    ...
    fn suggest_correction(tokens: &[&str]) {
        ...
        if tokens.contains(&"all") {
            ...
            println!(
                "   -> Did you mean: '{}'?",
                tokens.join(" ").replace("all", "web-service")
            );
        }
    }
}
...
async fn main() {
    ...
    frame
        .process_turn("deploy web-service on cluster with replicas 3")
        .await;
