...
impl MultiPartyLedger {
    ...
    fn simulate_consensus(
        ...
        println!(
            "[KERNEL] MOCWord bounds validated: c = {:.4}, Rsc = {:.4}",
            c, rsc
        );
...
fn verify_witness(
    ...
    for (i, (sig_bytes, pk_bytes)) in witness
        .signatures
        .iter()
        .zip(witness.public_keys.iter())
        .enumerate()
    {
...
fn main() {
    ...
    match ledger.simulate_consensus(&safe_word, action, 3) {
        ...
        Ok(witness) => {
            ...
            let payload = format!(
                "{}::{}::{}::{}",
                witness.moc_word_ast, witness.action, witness.c_bound, witness.rsc
            );
            match verify_witness(&witness, payload.as_bytes(), 3) {
                Ok(_) => {
                    let json = serde_json::to_string_pretty(&witness).unwrap();
                    println!(
                        "[VERIFIED] UnifiedWitness committed to immutable ledger:\n{}",
                        json
                    );
                }
    ...
    match ledger.simulate_consensus(&unsafe_word, bad_action, 3) {
        ...
        Err(e) => println!(
            "[LEDGER DENIED] Action never reached consensus phase.\nReason: {}",
            e
        ),
    }
}
