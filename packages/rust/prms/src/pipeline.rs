...
use crate::dae::{SimulationState, TelemetryFrame, TwoOscillatorEngine};
use crate::petc::{BytecodeHeader, ProofWitnessVerificationSystem};
use crate::zeta_ros::{AuditEngine, ComplianceBudget, LineageMetrics};
...
impl OperationalPipeline {
    ...
    pub fn execute_monitored_run(
        ...
        let (tx, rx): (SyncSender<TelemetryFrame>, Receiver<TelemetryFrame>) =
        ...
        let expected_hash =
            ProofWitnessVerificationSystem::compute_provenance_hash(&header, alpha, &prime_table);
        let mut history_ledger = Vec::with_capacity(self.config.steps);
        let mut processed_steps = 0;

        while let Ok(frame) = rx.recv() {
            let runtime_hash = ProofWitnessVerificationSystem::compute_provenance_hash(
                &header,
                alpha,
                &prime_table,
            );
            ...
            match AuditEngine::verify_step_lawfulness(
                &frame,
                &current_lineage,
                &budget,
                provenance_valid,
