...
fn proof_frame_cond_within_budget() {
    ...
    kani::assert(
        frame.cond_number <= budget.max_allowed_cond,
        "cond within budget",
    );
...
fn proof_witness_deterministic() {
    ...
    kani::assert(
        w1.telemetry_hash == w2.telemetry_hash,
        "Witness hash deterministic",
    );
    kani::assert(
        w1.cond_number == w2.cond_number,
        "Witness cond deterministic",
    );
}
