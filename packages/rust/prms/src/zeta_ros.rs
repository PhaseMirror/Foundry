...
impl AuditEngine {
    pub fn calculate_lineage_score(metrics: &LineageMetrics) -> (f64, f64, f64, f64) {
        let s_f = (-metrics.data_age_seconds / metrics.maximum_allowed_age)
            .exp()
            .min(1.0)
            .max(0.0);
        let s_c = if metrics.total_channels == 0 {
            0.0
        } else {
            (metrics.non_zero_channels as f64 / metrics.total_channels as f64)
                .min(1.0)
                .max(0.0)
        };
...
mod verification {
    ...
    fn verify_zeta_ros_veto_soundness() {
        ...
        match result {
            Err(reason) => kani::assert(
                reason == "FAIL-CLOSED: Cryptographic token mismatch on Path B audit.",
                "Must veto due to provenance mismatch",
            ),
