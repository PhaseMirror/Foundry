...
fn run_diagnostic(kappa: f64, gamma: f64, sigma: f64, z: f64, normalize: bool) {
    ...
    const N: usize = basis::N_STATES;
    let h1 = build_h1(&basis::NUMBERS, &basis::VALS, kappa, sigma);
    let h2 = build_h2(&basis::NUMBERS, &basis::VALS, kappa, gamma, sigma, normalize);
    let r1 = (Matrix::<N>::identity().scale(z).sub(&h1)).inv().unwrap();
    let r2 = (Matrix::<N>::identity().scale(z).sub(&h2)).inv().unwrap();
    let v2 = build_v2(&basis::NUMBERS, &basis::VALS, kappa, gamma, sigma, normalize);
