    ...
    if let (Some(j), Some((a_dag_list, a_list))) = (j_matrix, ladder_ops) {
        ...
        for i in 0..n_primes {
            for j_idx in (i + 1)..n_primes {
                if j[(i, j_idx)] != 0.0 {
                    let h_int = (&a_dag_list[i] * &a_list[j_idx] + &a_dag_list[j_idx] * &a_list[i])
                        * Complex64::new(j[(i, j_idx)], 0.0);
                    h += h_int;
                }
            }
        }
...
pub fn evaluate_zrsd_step(
    ...
    let term2 = (g_base * psi + r_zeta * psi * Complex64::new(epsilon, 0.0))
        * Complex64::new(lambda_m, 0.0);
