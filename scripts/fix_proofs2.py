...
for model in models:
    ...
    if os.path.exists(proofs_path):
        ...
        content = content.replace("ADRStatus.Accepted", "Core.ADR.ADRStatus.Accepted")
        content = content.replace("ADRStatus.Proposed", "Core.ADR.ADRStatus.Proposed")
        content = content.replace("ADRStatus.Deprecated", "Core.ADR.ADRStatus.Deprecated")
        content = content.replace("ADRStatus.Superseded", "Core.ADR.ADRStatus.Superseded")
        
        with open(proofs_path, "w") as f:
            f.write(content)

print("Fixed ADRStatus constructors in Proofs.lean for all models.")
