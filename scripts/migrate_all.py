...
def analyze_ensembles():
    ...
    for d in os.listdir(ensembles_dir):
        ...
        if has_cargo and not empty_lib:
            results[d] = path
            
    return results

valid_crates = analyze_ensembles()
...
for d, path in valid_crates.items():
    ...
    # fix path in Cargo.toml
    cargo_path = os.path.join(new_path, "Cargo.toml")
    if os.path.exists(cargo_path):
        with open(cargo_path, 'r') as f:
            content = f.read()
        
        # fix paths like ../trace-serializer -> ../trace_serializer
        # wait, we can just fix all ../ dependencies
        content = re.sub(r'path\s*=\s*"../([^"]+)"', lambda m: f'path = "../{m.group(1).replace(" ", "_").lower()}"', content)
        # handle specific rename for umc-parom to parom
        content = content.replace('"../umc-parom"', '"../parom"')
        
        with open(cargo_path, 'w') as f:
            f.write(content)
            
        # write generic artifacts
