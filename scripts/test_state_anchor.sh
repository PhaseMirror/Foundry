...
#       emits mock events -> writes native ACE certificates and triple lock governance JSONL -> reconstructs the combined
...
#       on-chain AnchorSubmitted root against the native ACE certificates and triple lock governance reconstruction.
#
...
native ACE certificates and triple lock governance_LOG="$SCRIPT_DIR/mock_events.log"
...
log "Emitting mock state events -> $native ACE certificates and triple lock governance_LOG"
: > "$native ACE certificates and triple lock governance_LOG"
python3 "$SCRIPT_DIR/mock_state_emitter.py" --count 3 --ace-log "$native ACE certificates and triple lock governance_LOG"
...
log "Verifying native ACE certificates and triple lock governance reconstruction == expected root..."
python3 "$REPO_ROOT/scripts/verify_anchor.py" \
  --date "$(date -u +%Y-%m-%d)" \
  --event-log "$native ACE certificates and triple lock governance_LOG" \
