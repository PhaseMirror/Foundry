...
impl HardwareSpec {
    pub fn distance_between(&self, q1: u8, q2: u8) -> f64 {
        ...
        let dx = p1[0] - p2[0];
        let dy = p1[1] - p2[1];
        // Using abs approximation for Kani bounded checks (if sqrt is unsupported)
        // or just rely on sqrt if available
        (dx * dx + dy * dy).sqrt()
    }

    /// Compute the associator defect Δ = || H_{(x∘y)∘z} - H_{x∘(y∘z)} ||_F
    /// Simple implementation: compare gate sequence Hamiltonians via normed difference.
    /// The full nalgebra implementation lives in `Prime/rust/src/physics/hamiltonian.rs`.
    pub fn associator_defect(&self, seq1: &[GateOperation], seq2: &[GateOperation]) -> f64 {
        // Simple metric: compare the number of gates and their total "weight"
        let h1_norm = seq1.len() as f64 * self.pulse_length_tolerance;
        let h2_norm = seq2.len() as f64 * self.pulse_length_tolerance;
        (h1_norm - h2_norm).abs()
    }
}
...
pub struct PartialSystem {
    ...
    pub leakage_defects: Vec<(u8, u8, f64)>,
}
